...
该脚本用于将mysql_to_postgres.py中的ETL任务转化为ETL任务记录并插入到MySQL数据库中。
在系统启动时自动执行，确保ETL任务已经被正确配置。
"""
...
def create_database_connections():
    ...
    try:
        ...
        (name, description, connection_type, host, port, username, password, database, config, created_at, updated_at) 
        ...
        return {
            ...
            "redis": cursor.lastrowid
        }
    except Exception as e:
        logger.error(f"创建数据库连接记录失败: {e}")
        sys.exit(1)

def create_etl_tasks(connection_ids):
    """
    创建ETL任务记录
    """
    try:
        ...
        # 检查是否已存在任务记录
        cursor.execute("SELECT COUNT(*) as count FROM etl_tasks")
        result = cursor.fetchone()
        ...
        for task in PREDEFINED_TASKS:
            ...
            if task["task_type"] == "mysql_to_postgres":
                source_connection_id = connection_ids["mysql"]
                target_connection_id = connection_ids["postgres"]
            elif task["task_type"] == "custom_sql":
                # 对于自定义SQL任务，根据配置中的target_connection设置目标连接
                target_connection = task["config"].get("target_connection")
                if target_connection and target_connection in connection_ids:
                    target_connection_id = connection_ids[target_connection]
            ...
            (name, description, task_type, source_connection_id, target_connection_id, config, schedule, status, created_at, updated_at) 
...
def main():
    ...
    try:
        ...
        # 创建数据库连接记录
        connection_ids = create_database_connections()
        
        # 创建ETL任务记录
        if connection_ids:
            create_etl_tasks(connection_ids)
