# PostgreSQL到Redis同步模块
# 负责PostgreSQL到Redis的数据同步
...
class PostgresToRedisETL:
    ...
    def execute(self, task: ETLTask) -> int:
        ...
        try:
            ...
            logger.error(f"PostgreSQL到Redis同步失败: {e}")
            raise
    ...
    def _validate_config(self, task: ETLTask, source_query: Optional[str], key_field: Optional[str]) -> None:
        ...
        if not source_query:
            # 尝试从任务名称中提取查询
            if task.name and 'postgres' in task.name.lower():
                # 使用默认查询
                source_query = f"SELECT * FROM public.users LIMIT 1000"
